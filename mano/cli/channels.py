"""Channel management CLI commands for manobot."""
...
def channels_status():
    ...
    # Matrix (if available)
    if hasattr(config.channels, "matrix"):
        mx = config.channels.matrix
        mx_config = mx.homeserver if hasattr(mx, "homeserver") and mx.homeserver else "[dim]not configured[/dim]"
        table.add_row(
            "Matrix",
            "✓" if mx.enabled else "✗",
            mx_config,
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".manobot" / "bridge"
