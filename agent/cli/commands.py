...
from agent import __logo__, __version__
from agent.config.paths import get_workspace_path
...
def _init_prompt_session() -> None:
    ...
    from agent.config.paths import get_cli_history_path
...
async def _print_interactive_line(text: str) -> None:
    ...
    def _write() -> None:
        ansi = _render_interactive_ansi(
            lambda c: c.print(f"  [dim]â†³ {text}[/dim]")
        )
...
def onboard(
    ...
    from agent.config.loader import get_config_path, load_config, save_config, set_config_path
    from agent.config.schema import Config
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from agent.config.loader import load_config, set_config_path
...
def gateway(
    ...
    from agent.config.paths import get_cron_dir
    ...
    cron_store_path = cron_root / "cron" / "jobs.json" if cron_root.name != "cron" else cron_root / "jobs.json"
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from agent.utils.evaluator import evaluate_response
...
def _get_bridge_dir() -> Path:
    ...
    from agent.config.paths import get_bridge_install_dir
...
def plugins_list():
    ...
    from agent.channels.registry import discover_all, discover_channel_names
    from agent.config.loader import load_config
