...
from agent.utils.helpers import ensure_dir, estimate_message_tokens, estimate_prompt_tokens_chain
...
async def _provider_chat(
    ...
    *,
    ...
    """Call the provider using either the new or legacy chat API."""
    from agent.providers.base import LLMProvider as _ProviderBase

    chat = getattr(provider, "chat", None)
    if not isinstance(provider, _ProviderBase) and callable(chat):
        return await chat(messages=messages, tools=tools, model=model)
    ...
    raise AttributeError("Provider does not expose chat_with_retry or chat")
...
class MemoryStore:
    ...
    async def consolidate(
        ...
        """Consolidate a message chunk or a session tail into MEMORY.md + HISTORY.md."""
        if hasattr(messages, "messages"):
            session = messages
            pending = session.messages[session.last_consolidated :]
            if memory_window is not None and len(pending) < memory_window:
                return True
            messages = pending
...
class MemoryConsolidator:
    ...
    async def archive_messages(self, messages: list[dict[str, object]]) -> bool:
        """Archive messages with guaranteed persistence (retries until fallback succeeds)."""
        if not messages:
            return True
