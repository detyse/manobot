...
from agent.agent.memory import MemoryConsolidator, MemoryStore
...
if TYPE_CHECKING:
    from agent.config.schema import ChannelsConfig, ExecToolConfig, WebSearchConfig
...
class AgentLoop:
    ...
    async def _handle_restart(self, msg: InboundMessage) -> None:
        """Request a process restart via the supervisor."""
        await self.bus.publish_outbound(OutboundMessage(
    ...
    def _schedule_background(self, coro) -> None:
        ...
        task.add_done_callback(lambda done: self._background_tasks.remove(done) if done in self._background_tasks else None)
