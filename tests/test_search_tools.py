...
async def test_glob_can_return_directories_only(tmp_path: Path) -> None:
    ...
    tool = GlobTool(workspace=tmp_path, allowed_dir=tmp_path)
    result = await tool.execute(pattern="api", path="src", entry_type="dirs")
...
def test_agent_loop_registers_grep_glob_and_exec_sandbox(tmp_path: Path) -> None:
    bus = MessageBus()
    provider = MagicMock()
    ...
    loop = AgentLoop(
        ...
        workspace=tmp_path,
        model="test-model",
        exec_config=ExecToolConfig(sandbox="bwrap"),
    )

    assert "grep" in loop.tools.tool_names
    assert "glob" in loop.tools.tool_names
    assert loop.tools.get("exec").sandbox == "bwrap"
    assert loop.tools.get("read_file")._allowed_dir == tmp_path


@pytest.mark.asyncio
async def test_subagent_registers_grep_glob_and_exec_sandbox(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    bus = MessageBus()
    ...
    def capture_register(self, tool):
    ...
    assert {"grep", "glob", "exec"} <= names
    assert next(tool for tool in registered if tool.name == "exec").sandbox == "bwrap"
    assert next(tool for tool in registered if tool.name == "read_file")._allowed_dir == tmp_path
