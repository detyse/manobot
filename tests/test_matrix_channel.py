...
matrix_module = pytest.importorskip("agent.channels.matrix", exc_type=ImportError)
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("agent.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "agent.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("agent.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "agent.channels.matrix.asyncio.create_task", _fake_create_task
    )
